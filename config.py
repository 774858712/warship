...
class LoadConfig(object):
    def __init__(self, args, version):
        ...
        if args.dataset == 'product':
            ...
            self.numcls = 100
        elif args.dataset == 'Warship':
            self.dataset = args.dataset
            self.rawdata_root = './dataset/ship/data'
            self.anno_root = './dataset/ship/anno'
            self.numcls = 40
        else:
        ...
        if 'test' in get_list:
            self.test_anno = pd.read_csv(os.path.join('D:/DCL', 'ct_val.txt'),\
                                           sep=" ",\
                                           header=None,\
                                           names=['ImageName', 'label'])
            self.rawdata_root = 'D:/DCL/all_img'

        '''if 'test' in get_list:
            self.test_anno = pd.read_csv(os.path.join('D:/DCL', 'ct_test1.txt'),\
                                           sep=" ",\
                                           header=None,\
                                           names=['ImageName', 'label'])
            self.rawdata_root = 'D:/DCL/all_test'''
